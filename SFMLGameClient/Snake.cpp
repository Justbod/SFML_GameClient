...
void Snake::Reset()
{
	...
	SetDirection(Direction::None); //start of standing still
	m_speed = 15;
	m_lives = 3;
	m_score = 0;
	m_lost = false;
...
void Snake::Extend()
{
	if (m_snakeBody.empty()) { return; } //if snake has no body return
	SnakeSegment& tail_head = m_snakeBody[m_snakeBody.size() - 1];

	if (m_snakeBody.size() > 1) { //if snake size is bigger then 1
